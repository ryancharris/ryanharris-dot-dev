...
Last time, we learned about the <Link to="/hooks-useeffect">useEffect hook</Link>, how it works and when to use it. If you have not read that article yet, I **strongly** suggest you go back and do so before proceeding any further. Much of what we will discuss below will be about the similiarities and difference between `useEffect` and `useLayoutEffect`, which may not make sense without having a grip on `useEffect`.
...
As we saw above, these two hooks are quite similar in terms of their syntax and function signatures, however, the difference between them is quite subtle and has everything to do with timing. Let's dive a bit deeper to get a better understanding of how they work.
...
In the example below, we have a simple application that renders a few HTML elements, namely two `div`s and a `main`, which we can see in `App.js`:
...
Above `<App />`'s return, you will see two effects, one defined with `useEffect` and the other with `useLayoutEffect`.
...
<iframe
     src="https://codesandbox.io/embed/hooksuselayouteffect-vgkbd?fontsize=14&hidenavigation=1&theme=dark"
...
To start, comment out the `useLayoutEffect` hook and refresh the sandbox. You should see the purple circle appear in the top left-hand corner of the screen before it is quickly repositioned and moved to the center of the screen. This happens because **useEffect runs after render**, so the effect isn't executed till after the DOM paints, which is what causes the unwanted flash of content.

Now, let's reverse things. Uncomment the `useLayoutEffect` hook and instead comment out our `useEffect`. Now, if you refresh the page, you should always see the purple circle in the center of the screen and no longer see the circle get quickly repositioned. This is because **useLayoutEffect runs before the DOM pains**, so the circle has already been positioned correctly before we see the first visuals on our page. In this example, it would be more appropriate to use `useLayoutEffect` because we would not want our users to see what looks like a visual bug.

In the snippets above, we are using vanilla JavaScript to access and modify the document, which is considered an anti-pattern in React. A more appropriate way to do this would be to use a `ref` instead of accessing the DOM directly. Luckily, we'll be covering that in the next article on `useRef`!
