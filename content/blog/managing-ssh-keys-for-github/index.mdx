...
1. Create two SSH keys/identities and manage them locally usings `ssh-agent`
...
Now, whenever we `push` or `pull` from GitHub, the proper key and identity will be used based upon this repo's remote `origin`. Since we cloned our repos using the altered URLs appended with our user name, they will automatically work.

Repos you cloned prior to having two SSH identities, however, will not work right away. Instead, you simply need to update their remote `origin` URL to use the `-username` suffix. Then, the SSH config will know which key to associate with the request.
