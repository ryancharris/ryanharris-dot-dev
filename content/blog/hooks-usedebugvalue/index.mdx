...
In the snippet above, we've defined our own custom hook, `useLog` (more about these in an upcoming article). Currently, it just logs the current date to the React dev console the first time our `App` component renders due to the empty dependency array.
