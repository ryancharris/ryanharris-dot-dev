...
In the snippet above, we've defined our own custom hook, `useLog` (more about these in an upcoming article). Currently, it just logs `"hi"` to the dev console the first time our `App` component renders due to the empty dependency array.

Note that we're also using a `useDebugValue` hook within `useLog`, which takes a single value and then displays it in the [React DevTools](https://github.com/facebook/react-devtools). In the screenshot below, you can see how the snippet above would look in your browser.
