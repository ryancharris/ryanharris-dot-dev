...
First, let's look at what is happening on the right side of this expression. Here, we have the value of `10` being passed as the only argument to our `useState` hook. This sets its inital value to `10`.
...
The second value in the array (i.e. `setApples`) is a setter function that allows you to update the value of `apples` by calling `setApples(200)`, which would update `apple` to be equal to `200`.
