...
Essentially, to keep things performany and properly manage memory, React may get rid of a cached value its not actively using in order to save room for other operations. In this case, `useMemo` will have to recalculate its return value even if the values being passed to the hook have been used before.
