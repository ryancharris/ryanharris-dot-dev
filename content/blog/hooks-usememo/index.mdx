...
First, look at the dependency array (i.e. `[inputValue, nasaLocations]`). Like `useEffect`, this tells `useMemo` only to run when either of those values change. Right now, we only make a call for `nasaLocation` on initial render, so its value will only change once, which triggers the hook.
